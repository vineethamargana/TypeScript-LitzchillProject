 ...
 */
 async function createMeme(req: Request, params: Record<string, string>, uploadFileToBucketQuery = uploadFileToBucket, CreateMemeQueryFn = createMemeQuery): Promise<Response> {
    ...
    if (!validateContentType) {
        ...
        throw new CustomException(HTTP_STATUS_CODE.BAD_REQUEST, MEME_ERROR_MESSAGES.MISSING_REQUIRED_FEILDS);
    }
    ...
    if (!uploadedUrl) {
        ...
        throw new CustomException(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, MEME_ERROR_MESSAGES.MEDIA_UPLOAD_FAILED);
    }
    ...
    if (insertError) {
        ...
        throw new CustomException(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, MEME_ERROR_MESSAGES.FAILED_TO_CREATE);
    }
