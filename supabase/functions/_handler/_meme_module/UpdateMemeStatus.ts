 ...
 async function updateMemeStatus(req: Request, params: Record<string, string>, updateMemeStatusQueryFn = updateMemeStatusQuery) {
    const logger = Logger.getInstance();
      ...
      if (!meme_status) {
          ...
          throw new CustomException(HTTP_STATUS_CODE.BAD_REQUEST, MEME_ERROR_MESSAGES.MISSING_STATUS_VALUE);
      }
      ...
      if (!validStatuses.includes(meme_status)) {
          ...
          throw new CustomException(HTTP_STATUS_CODE.BAD_REQUEST, COMMON_ERROR_MESSAGES.INVALID_DATA);
      }

      // Update meme status
      const { data: updatedMemeStatus, error } = await updateMemeStatusQueryFn(meme_id, meme_status,user_id);
      if (error || !updatedMemeStatus) {
          logger.error("Database update failed:"+ JSON.stringify(error));
          throw new CustomException(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, MEME_ERROR_MESSAGES.FAILED_TO_UPDATE);
      }
