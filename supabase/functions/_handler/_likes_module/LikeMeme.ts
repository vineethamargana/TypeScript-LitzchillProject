...
import ErrorHandler from "@shared/ExceptionHandling/ErrorHandler.ts";
...
async function likememe(_req: Request, params: Record<string, string>,CheckMemeExists = meme_exists, likememeQuery = insertLikeQuery) {
    ...
    if (!meme_id || !V4.isValid(meme_id)) { 
        ...
        throw new CustomError(HTTP_STATUS_CODE.BAD_REQUEST, MEME_ERROR_MESSAGES.MISSING_MEMEID);
    }
    ...
    if (!existingMeme) {
        ...
        throw new CustomError(HTTP_STATUS_CODE.NOT_FOUND, MEME_ERROR_MESSAGES.MEME_NOT_FOUND);
    }
    ...
    if (likeError) {
        ...
        throw new CustomError(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, LIKE_ERROR.INSERTION_FAILED);
    }
