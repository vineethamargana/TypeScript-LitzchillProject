 ...
 */
 async function markNotification(_req: Request, params: Record<string, string>, markNotificationsAsRead = markNotificationsAsReadQuery): Promise<Response> {
        ...
        if (!notification_id || !V4.isValid(notification_id)) {
            ...
            throw new CustomException(HTTP_STATUS_CODE.BAD_REQUEST, NOTIFICATION_ERRORS.MISSING_ID);
        }
        ...
        if (!isSuccessful) {
            ...
            throw new CustomException(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, NOTIFICATION_ERRORS.FAILED_TO_UPDATE);
        }
